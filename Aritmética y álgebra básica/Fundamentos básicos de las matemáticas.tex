...
\begin{document}
    ...
    \section{Derivadas y aplicaciones}
        ...
        \subsection*{Idea clave}
            Si tenemos una función $y= f(x)$
            \[ f'(x) = \lim_{h \longrightarrow 0} \frac{f(x+h)-f(x)}{h}\]
            \begin{itemize}
                \item Este se llama \textbf{definición formal de la derivada}
                \item Básicamente comparamos \textbf{el cambio de $y$ dividido por el cambio de $x$} y hacemos que $x+h$ se acerque a $x$.
            \end{itemize}
        ...
        \subsection*{Ejemplo}
            ...
            \[ f'(x) = \lim_{h \longrightarrow 0} \frac{(x+h)^2 - x^2}{h}\]
            Expandiendo:
            \[(x+h)^2 -x^2 = x^2+2xh+h^2-x^2  = 2xh+h^2\]
            Dividimos entre h:
            \[\frac{2xh+h^2}{h} = 2x+h\]
            Tomamos el límite cuando $h \longrightarrow 0$:
            \[f'(x)=2x\]
            Entonces la derivada de $x^2$ es $2x$
